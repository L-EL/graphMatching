...
def extractOrganOrientation(img, bgLabel):
	borderLabels = np.unique(np.concatenate([np.unique(img[:,:,-1]),np.unique(img[:,-1,:]),np.unique(img[-1,:,:]),np.unique(img[:,:,0]),np.unique(img[0,:,:]),np.unique(img[:,0,:])]))
...
segmentationFileName1 = "" #path to a tif (segmentation)

segmentationFileName2 = "" # path to a second tif (segmentation)
outImgFolder = segmentationFileName1[:segmentationFileName1.rfind(".")]
dz,dx,dy = read_tiff_voxel_size(segmentationFileName1)

label1 = [183] #label in segmentation 1
label2 = [878] # label in segmentation 2
seeds = [[label1[0],label2[0]]]
	

imgSeg1 = skimage.io.imread(segmentationFileName1)
imgSeg2 = skimage.io.imread(segmentationFileName2)

imC1 = label2Contours3D(imgSeg1)
imC2 = label2Contours3D(imgSeg2)

vecOrgan1 = extractOrganOrientation(imgSeg1, np.median(imgSeg1))
vecOrgan2 = extractOrganOrientation(imgSeg2, np.median(imgSeg2))
...
### iterative solving second trial : with iteration on the graph istead of the affinity matrix 
assignedNodes1 = [seeds[0][0]]
assignedNodes2 = [seeds[0][1]]
assignedNumberPrev =  0
iteration = 0
while (len(assignedNodes1)!= len(labels1)) and (len(assignedNodes2)!= len(labels2)) and (len(assignedNodes1) != assignedNumberPrev):
	...
	g1Sub = g1.subgraph(toAssigned1) ##here can remove the already assigned nodes except the ones  of the last iteration TODO
	...
	for link in conn1:
		...
		vec = np.array([rp1[iNe].centroid[0]*dz-rp1[iNeOri].centroid[0]*dz,rp1[iNe].centroid[1]*dx-rp1[iNeOri].centroid[1]*dx,rp1[iNe].centroid[2]*dy-rp1[iNeOri].centroid[2]*dy])
	...
	for link in conn2:
		...
		vec = np.array([rp2[iNe].centroid[0]*dz-rp2[iNeOri].centroid[0]*dz,rp2[iNe].centroid[1]*dx-rp2[iNeOri].centroid[1]*dx,rp2[iNe].centroid[2]*dy-rp2[iNeOri].centroid[2]*dy])
		normVec =  np.sqrt(vec[0]**2 +vec[1]**2+vec[2]**2)
		angles = np.arccos(np.clip(np.dot(vec.T,vecImg)/ normVec,-1.0, 1.0))
	...
	for mN in matchNodes:
		...
		matchLabelSeg[xyz[:,0],xyz[:,1],xyz[:,2]] = labels1Sub[mN[0]]
	if showOn:
		plt.show()
	...
	iteration+=1
tifffile.imwrite(outImgFolder+"segMatch"+str(iteration)+".tif", matchLabelSeg) #ok
np.save(outImgFolder+'correspondingLabels.npy',[assignedNodes1,assignedNodes2])
